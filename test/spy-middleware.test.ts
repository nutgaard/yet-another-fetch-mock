...
describe('SpyMiddlware', () => {
  ...
  it('should capture all calls', async () => {
    await Promise.all([
  ...
  it('should return true if route has been used by fetchmock', async () => {
    await Promise.all([
  ...
  it('should return the last url matched', async () => {
    await Promise.all([
      fetchToJson('/data/213'),
  ...
  it('should return the last options matched', async () => {
    await Promise.all([
  ...
  it('should find all matching calls', async () => {
    await Promise.all([
      fetchToJson('/data/213'),
      fetchToJson('/data/ignore', { method: 'POST' }),
      fetchToJson('/data/214'),
  ...
  it('should capture calls', async () => {
    mock.get('/test/:id', (req, res, ctx) => res(ctx.json({ data: 'test' })));
    await Promise.all([fetch('/test/121'), fetch('/test/122')]).then(() => {
  ...
  it('should remove all entries on reset', async () => {
    mock.get('/test/:id', (req, res, ctx) => res(ctx.json({ data: 'test' })));
    await Promise.all([fetch('/test/121'), fetch('/test/122')]).then(() => {
      expect(spy.size()).toBe(2);
